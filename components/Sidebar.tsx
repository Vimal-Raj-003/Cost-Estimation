...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, toggleSidebar, onNewEstimate, onViewChange, activeView }) => {
  const menuItems = [
    { id: 'dashboard', icon: LayoutDashboard, label: 'Dashboard' },
    { id: 'new-estimate', icon: PlusCircle, label: 'New Estimate', action: onNewEstimate },
    { id: 'estimates', icon: List, label: 'Estimates' },
    { id: 'templates', icon: FileText, label: 'Templates' },
    { id: 'rate-cards', icon: Database, label: 'Rate Cards' },
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto">
        {menuItems.map((item) => {
          const isActive = activeView.includes(item.id);
          return (
            <button
              key={item.id}
            ...
            >
              <item.icon className={`w-5 h-5 ${isActive ? 'text-white' : 'text-slate-500 group-hover:text-slate-100'}`} />
